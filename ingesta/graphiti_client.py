...
Toda la API pública es async — ingest.py debe llamarla con await dentro de
un async def main() ejecutado con asyncio.run().
...
class GraphitiClient:
    ...
    Toda la API pública es async. Cuando graphiti-core no está disponible
    o Neo4j está caído, cae automáticamente a SQLite local.
    """
    ...
    def __init__(
        ...
        self._neo4j_uri = neo4j_uri or os.environ.get("NEO4J_URI", _DEFAULT_NEO4J_URI)
        self._neo4j_user = neo4j_user or os.environ.get("NEO4J_USER", _DEFAULT_NEO4J_USER)
        self._neo4j_password = neo4j_password or os.environ.get("NEO4J_PASSWORD", _DEFAULT_NEO4J_PASSWORD)
    ...
    # ------------------------------------------------------------------
    # SQLite helpers (síncronos — sqlite3 no tiene API async)
    # ------------------------------------------------------------------
    ...
    def _sqlite_write_rel(self, from_id: str, rel_type: str, to_id: str, props: dict) -> None:
        self._sqlite_conn.execute(
    ...
    # ------------------------------------------------------------------
    # graphiti-core helpers (async nativos — sin _run_sync)
    # ------------------------------------------------------------------
    ...
    async def _get_graphiti(self):
        ...
        try:
            ...
            logger.warning("[graphiti-client] graphiti-core no instalado — usando SQLite fallback.")
    ...
    async def health_check(self) -> bool:
        ...
        return await self._get_graphiti() is not None
    ...
    async def create_node(self, label: str, properties: Dict[str, Any]) -> str:
        ...
        if g:
            try:
                ...
                sanitized = _sanitize(properties)
                ...
                logger.warning(
                    "[graphiti-client] Error escribiendo nodo %s en Neo4j: %s — cayendo a SQLite.",
                    label, exc,
                )
                self._graphiti_available = False
    ...
    async def create_relationship(
        ...
        if g:
            try:
                ...
                logger.warning(
                    "[graphiti-client] Error escribiendo relación %s en Neo4j: %s — cayendo a SQLite.",
                    rel_type, exc,
                )
                self._graphiti_available = False
    ...
    async def add_episode(
        ...
        try:
            ...
            logger.debug("[graphiti-client] add_episode '%s' falló (no crítico): %s", name, exc)
            return False
    ...
    # ------------------------------------------------------------------
    # Introspección (stats para el summary — síncronos sobre SQLite)
    # ------------------------------------------------------------------
