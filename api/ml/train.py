...
def evaluate_model(
    ...
    """
    evalua un modelo con métricas completas.
    Retorna dict con precision, recall, f1, roc_auc, accuracy.
    """
    X_s = scaler.fit_transform(X)
    cv = StratifiedKFold(n_splits=_get_cv_splits(y), shuffle=True, random_state=42)

    y_pred = cross_val_predict(model, X_s, y, cv=cv)

    try:
        y_proba = cross_val_predict(model, X_s, y, cv=cv, method="predict_proba")
        idx_pos = 1 if y.ndim == 1 else list(model.classes_).index(1)
        if y_proba.ndim == 2:
...
def compare_models(X: np.ndarray, y_gt: np.ndarray | None) -> dict[str, Any]:
    ...
    for name, model_factory in _MODELS.items():
        ...
        if n >= 30:
            metrics = evaluate_model(model, X_labeled, y_labeled, scaler)
        else:
            metrics = {
                "precision": 0,
                "recall": 0,
    ...
    if n >= 30:
        ...
        gb_base = _MODELS[best_name]()
        model = CalibratedClassifierCV(gb_base, method="isotonic", cv=n_cv)
    ...
    fold_scores = []
    if n >= 30 and cv_splits >= 2:
        cv = StratifiedKFold(n_splits=cv_splits, shuffle=True, random_state=42)
        base_model = best_model_factory()
        fold_scores = list(
            cross_val_score(
                base_model,
                scaler.fit_transform(X_labeled),
                y_labeled,
                cv=cv,
                scoring="roc_auc",
            )
    ...
    return {
        ...
        "model_info": {
            "modelo": best_name,
            "calibration": "isotonic" if n >= 30 else "none",
            "label_mode": label_mode,
            "n_labeled": int(n),
...
def build_model(
    ...
    if len(y) >= 30:
        ...
        model = CalibratedClassifierCV(base, method="isotonic", cv=n_cv)
...
def run_cv_scoring(
    ...
    """Wrapper para cross_val_score con scaler predefinido."""
    X_s = scaler.fit_transform(X)
    cv = StratifiedKFold(n_splits=_get_cv_splits(y), shuffle=True, random_state=42)
    from sklearn.model_selection import cross_val_score as cv_score

    return list(cv_score(model, X_s, y, cv=cv, scoring=scoring))
